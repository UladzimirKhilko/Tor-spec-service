...
// Запасной текст для списка патрубков ("Назначение патрубков" — новый блок
// справа от картинки "Общий вид", см. modelExtract.js:
// extractPortLegendFromPdf) — подставляется, только если на загруженном PDF-
// бланке этот текст не удалось распознать (нестандартная вёрстка). Текст —
// самый частый случай (4 патрубка, одноходовой аппарат); для моноблочных
// аппаратов (6 патрубков) при неудачном распознавании сотруднику нужно будет
// дописать 2 строки вручную — статус-сообщение (app.js) явно предупреждает
// об этом при подстановке запасного текста.
const DEFAULT_PORT_LEGEND_TEXT = [
