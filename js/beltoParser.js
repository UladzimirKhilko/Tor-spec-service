...
// ВАЖНО: единица измерения иногда содержит цифру внутри самого обозначения
// (например "м3/ч" — кубометры в час, "м2" — квадратные метры) — без
// разделяющего пробела от буквы. Без учёта этого регулярка находила такую
// "3"/"2" как ОТДЕЛЬНОЕ число из середины строки: на реальном документе это
// давало и неверные значения (лишнее число в массиве сдвигало реальные
// значения при slice(-N) в extractValuesForRule ниже), и мусор в единице
// измерения (после вычитания "3" из "м3/ч" оставалось разбитое "м /ч") —
// то, что пользователь увидел как "лишние символы" в колонке "Ед.изм".
// Поэтому число ищем только там, где оно НЕ приклеено вплотную к букве
// (кириллица/латиница) ни слева, ни справа — настоящие значения в отчёте
// BelTO всегда отделены от единицы измерения пробелом.
const NUMBER_RE = /(?<![a-zA-Zа-яА-ЯёЁ])-?\d+(?:[.,]\d+)?(?![a-zA-Zа-яА-ЯёЁ])/g;
...
function extractUnitFromLine(line, labelMatchText, vals) {
  ...
  const tokens = residual
    ...
    .map((t) => t.trim())
    .filter((t) => t && t !== '-' && t !== '—' && t !== '.' && t !== ':')
    // Обрезок числа, не полностью вычтенный выше — например, если в
    // исходном тексте значение записано с хвостовыми нулями ("21.00"), а
    // распознанное число (n=21, см. цикл выше) при обратном переводе в
    // строку теряет их (String(21) === "21"): из residual вычитается
    // только "21", а ".00" остаётся и раньше попадал в единицу измерения
    // как мусор (например "т/ч .00" вместо "т/ч"). Настоящая единица
    // измерения никогда не состоит ТОЛЬКО из цифр/точки/запятой — отсекаем
    // такие "хвосты" отдельным фильтром, а не пытаясь угадать все варианты
    // форматирования числа заранее.
    .filter((t) => !/^-?[\d.,]+$/.test(t));
